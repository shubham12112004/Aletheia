...
const watchlist = [
  { ticker: 'AAPL', name: 'Apple Inc.', price: '$228.68', change: '+1.24%', score: 88, points: [22, 28, 24, 33, 38, 36, 44] },
  { ticker: 'TSLA', name: 'Tesla Inc.', price: '$252.41', change: '-0.82%', score: 71, points: [40, 35, 42, 31, 37, 34, 32] },
  { ticker: 'NVDA', name: 'NVIDIA Corp.', price: '$139.91', change: '+2.18%', score: 93, points: [18, 24, 31, 38, 42, 47, 55] },
  { ticker: 'MSFT', name: 'Microsoft Corp.', price: '$497.22', change: '+0.64%', score: 84, points: [28, 30, 34, 33, 39, 42, 45] },
];
    ...
    <div className="min-h-screen bg-[#F8FAFC] text-slate-950">
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1360px] items-center justify-between px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-500/25"><BrainCircuit className="h-5 w-5" /></div><div><h1 className="text-base font-black tracking-tight">AI Investment Research</h1><p className="text-xs text-slate-500">Multi-agent market intelligence</p></div></div>
...
function ResearchDashboard({ snapshot }: { snapshot: ResearchSnapshot }) {
  const { result, profile, financials, newsData, rawMarkdown, timelineCount } = snapshot;
  const derivedMetrics = useMemo(() => getDerivedMetrics(profile, financials), [profile, financials]);
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
    <div className="grid gap-5 xl:grid-cols-[0.8fr_1.2fr]"><DashboardCard delay={0.01}><SectionTitle title="Agent Map Network Graph" subtitle="Latest graph state from the research lifecycle" /><div className="mt-4 overflow-hidden rounded-2xl border border-slate-200 bg-slate-950"><ResearchGraphView steps={[]} /></div></DashboardCard>
      <div className="grid gap-5 lg:grid-cols-2"><DashboardCard delay={0.02}><div className="flex items-start gap-4"><div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-blue-600 to-sky-400 text-lg font-black text-white shadow-lg shadow-blue-500/20">{(profile?.ticker || result.ticker || 'AI').slice(0, 4)}</div><div className="min-w-0 flex-1"><p className="text-xs font-black uppercase tracking-[0.22em] text-slate-400">Company Overview</p><h3 className="mt-1 truncate text-2xl font-black tracking-tight text-slate-950">{profile?.name || result.company}</h3><div className="mt-3 grid grid-cols-2 gap-3 text-sm sm:grid-cols-4"><Info label="Symbol" value={profile?.ticker || result.ticker || 'N/A'} /><Info label="Industry" value={profile?.finnhubIndustry || 'Technology'} /><Info label="Market Cap" value={derivedMetrics.marketCap} /><Info label="Country" value={profile?.country || 'US'} /></div></div></div></DashboardCard>
        <DashboardCard delay={0.06}><div className="flex h-full flex-col justify-between gap-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-slate-400">AI Recommendation</p><div className="mt-3"><RecommendationBadge verdict={result.verdict} /></div></div><div className="rounded-2xl bg-blue-50 p-3 text-blue-700"><TrendingUp className="h-6 w-6" /></div></div><div className="grid grid-cols-3 gap-3"><Score label="Confidence" value={result.confidence || 80} color="blue" /><Score label="Risk" value={Math.max(10, 100 - (result.confidence || 80))} color="red" /><Score label="Growth" value={Math.min(98, (result.confidence || 80) + 8)} color="green" /></div></div></DashboardCard></div></div>
    <div className="grid gap-5 lg:grid-cols-2"><DashboardCard delay={0.1}><SectionTitle title="Financial Summary" subtitle="Key valuation and market indicators" /><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">{derivedMetrics.items.map((item) => <MetricBox key={item.label} {...item} />)}</div></DashboardCard><DashboardCard delay={0.14}><SectionTitle title="Market Sentiment" subtitle={`${timelineCount} backend research events processed`} /><div className="mt-4 grid grid-cols-3 gap-3"><Sentiment label="Positive" value={Math.max(result.pros?.length || 0, 1)} className="bg-emerald-50 text-emerald-700" /><Sentiment label="Neutral" value={3} className="bg-blue-50 text-blue-700" /><Sentiment label="Negative" value={Math.max(result.cons?.length || 0, 1)} className="bg-red-50 text-red-700" /></div><p className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm leading-6 text-slate-600 line-clamp-3">{result.executiveSummary?.[0] || 'The market sentiment view will update as the AI research completes.'}</p></DashboardCard></div>
    <DashboardCard delay={0.18}><DashboardCharts result={result} /></DashboardCard>
    <DashboardCard delay={0.22}><SectionTitle title="Latest News" subtitle="Top sources used by the research agent" /><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{(news.length ? news : fallbackNews).map((item, index) => <NewsCard key={`${item.title}-${index}`} {...item} />)}</div></DashboardCard>
    <DashboardCard delay={0.26} className="mb-8"><SectionTitle title="Complete AI Investment Report" subtitle="Structured AI report rendered from research output" /><div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50/70 p-5"><MarkdownReport markdown={rawMarkdown || buildFallbackMarkdown(result)} /></div></DashboardCard>
  </motion.div>;
}
...
function HistoryPane({ history, selectedId, onLoad, onRun }: { history: ResearchSnapshot[]; selectedId: string | null; onLoad: (id: string) => void; onRun: (ticker: string) => void }) {
  return <DashboardCard><SectionTitle title="Research History" subtitle="Previous ticker lookups are preserved locally for fast reloads" /><div className="mt-5 grid gap-3">{history.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-8 text-center text-sm font-semibold text-slate-500">No research history yet. Run a company search from the Dashboard tab.</div> : history.map((item) => <div key={item.id} className={cn('flex flex-col gap-3 rounded-2xl border p-4 sm:flex-row sm:items-center sm:justify-between', selectedId === item.id ? 'border-blue-300 bg-blue-50' : 'border-slate-200 bg-slate-50')}><div className="min-w-0"><div className="flex items-center gap-2"><span className="rounded-full bg-slate-900 px-2.5 py-1 text-xs font-black text-white">{item.ticker}</span><h3 className="truncate text-base font-black text-slate-950">{item.company}</h3></div><p className="mt-2 flex items-center gap-1 text-xs font-semibold text-slate-500"><Clock3 className="h-3.5 w-3.5" /> {new Date(item.createdAt).toLocaleString()}</p></div><div className="flex gap-2"><Button onClick={() => onLoad(item.id)} variant="outline" className="rounded-full bg-white font-bold">Load Dataset</Button><Button onClick={() => onRun(item.ticker)} className="rounded-full bg-blue-600 font-bold text-white hover:bg-blue-700">Run Again</Button></div></div>)}</div></DashboardCard>;
}

function WatchlistPane({ onResearch }: { onResearch: (ticker: string) => void }) {
  return <div className="space-y-5"><DashboardCard><SectionTitle title="Watchlist" subtitle="Monitor key tickers and trigger deep research instantly" /></DashboardCard><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">{watchlist.map((asset) => <DashboardCard key={asset.ticker}><div className="flex items-start justify-between gap-3"><div><p className="text-xl font-black text-slate-950">{asset.ticker}</p><p className="text-sm font-semibold text-slate-500">{asset.name}</p></div><span className={cn('rounded-full px-2.5 py-1 text-xs font-black', asset.change.startsWith('+') ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700')}>{asset.change}</span></div><div className="mt-4"><MiniSparkline points={asset.points} positive={asset.change.startsWith('+')} /></div><div className="mt-4 grid grid-cols-2 gap-3"><MetricBox label="Price" value={asset.price} /><MetricBox label="AI Score" value={`${asset.score}%`} /></div><Button onClick={() => onResearch(asset.ticker)} className="mt-4 w-full rounded-full bg-blue-600 font-bold text-white hover:bg-blue-700"><PlayCircle className="mr-2 h-4 w-4" /> Deep Research</Button></DashboardCard>)}</div></div>;
}
function SettingsPane({ apiEndpoint, setApiEndpoint, analysisDepth, setAnalysisDepth, quotaAlerts, setQuotaAlerts, socketProgress, setSocketProgress }: { apiEndpoint: ApiEndpoint; setApiEndpoint: (value: ApiEndpoint) => void; analysisDepth: AnalysisDepth; setAnalysisDepth: (value: AnalysisDepth) => void; quotaAlerts: boolean; setQuotaAlerts: (value: boolean) => void; socketProgress: boolean; setSocketProgress: (value: boolean) => void }) {
  return <div className="grid gap-5 lg:grid-cols-[1fr_0.8fr]"><DashboardCard><SectionTitle title="Workspace Configuration" subtitle="Configure API routing, reasoning depth, and realtime behavior" /><div className="mt-5 space-y-4"><SettingBlock icon={Globe2} title="API Endpoint" subtitle="Switch between production and staging integrations"><SegmentedControl value={apiEndpoint} options={[[ 'production', 'Production' ], [ 'staging', 'Staging' ]]} onChange={(value) => setApiEndpoint(value as ApiEndpoint)} /></SettingBlock><SettingBlock icon={BrainCircuit} title="Multi-Agent Reasoning Depth" subtitle="Fast uses fewer reasoning passes; Deep expands analysis and citations"><SegmentedControl value={analysisDepth} options={[[ 'fast', 'Fast Analysis' ], [ 'deep', 'Deep Analysis' ]]} onChange={(value) => setAnalysisDepth(value as AnalysisDepth)} /></SettingBlock><SettingBlock icon={SlidersHorizontal} title="Execution Controls" subtitle="Tune realtime progress and quota alerts"><div className="grid gap-3 sm:grid-cols-2"><Toggle label="Socket.IO progress events" checked={socketProgress} onChange={setSocketProgress} /><Toggle label="API quota alerts" checked={quotaAlerts} onChange={setQuotaAlerts} /></div></SettingBlock></div></DashboardCard><DashboardCard><SectionTitle title="API Usage Quotas" subtitle="Workspace quota overview" /><div className="mt-5 space-y-4"><Quota label="Gemini" value={62} /><Quota label="Financial Modeling Prep" value={44} /><Quota label="NewsAPI" value={71} /><Quota label="Tavily" value={38} /></div></DashboardCard></div>;
}

function ProfileModal({ user, tokenState, onClose }: { user: any; tokenState: string; onClose: () => void }) {
  const rows = [[ 'Name', user?.name || 'Unknown' ], [ 'Email', user?.email || 'Unknown' ], [ 'Provider', user?.provider || 'email' ], [ 'Session Token', tokenState ], [ 'Schema', 'User { email, name, provider }' ]];
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><motion.div initial={{ opacity: 0, y: 12, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="w-full max-w-lg rounded-3xl bg-white p-5 shadow-2xl"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.22em] text-blue-600">Authenticated Profile</p><h3 className="mt-1 text-xl font-black text-slate-950">User Metadata</h3></div><Button onClick={onClose} variant="ghost" size="icon" className="rounded-full"><X className="h-4 w-4" /></Button></div><div className="mt-5 overflow-hidden rounded-2xl border border-slate-200">{rows.map(([label, value]) => <div key={label} className="grid grid-cols-[130px_1fr] border-b border-slate-100 last:border-0"><div className="bg-slate-50 px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-400">{label}</div><div className="min-w-0 break-words px-4 py-3 text-sm font-semibold text-slate-700">{value}</div></div>)}</div></motion.div></div>;
}
...
function EmptyDashboard() { return <div className="grid gap-5 lg:grid-cols-2"><DashboardCard><SectionTitle title="Company Overview" subtitle="Search for a company to begin" /><div className="mt-6 h-32 rounded-2xl border border-dashed border-slate-200 bg-slate-50" /></DashboardCard><DashboardCard><SectionTitle title="Stock Evaluation Cards" subtitle="Recommendation cards appear when research completes" /><div className="mt-6 h-32 rounded-2xl border border-dashed border-slate-200 bg-slate-50" /></DashboardCard></div>; }
function MiniSparkline({ points, positive }: { points: number[]; positive: boolean }) { const max = Math.max(...points); const min = Math.min(...points); const coords = points.map((point, index) => `${(index / (points.length - 1)) * 180},${52 - ((point - min) / Math.max(1, max - min)) * 44}`).join(' '); return <svg viewBox="0 0 180 60" className="h-16 w-full"><polyline fill="none" stroke={positive ? '#10b981' : '#ef4444'} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" points={coords} /></svg>; }
...
function Quota({ label, value }: { label: string; value: number }) { return <div><div className="flex items-center justify-between text-sm"><span className="font-bold text-slate-700">{label}</span><span className="font-black text-slate-950">{value}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-600" style={{ width: `${value}%` }} /></div></div>; }
...
function getDerivedMetrics(profile: any, financials: any) { const formatNumber = (val: any, prefix = '', suffix = '') => { if (val === undefined || val === null || Number.isNaN(Number(val))) return 'N/A'; return `${prefix}${Number(val).toLocaleString(undefined, { maximumFractionDigits: 2 })}${suffix}`; }; const metrics = financials?.metric || {}; const marketCapRaw = profile?.marketCapitalization; const marketCapDisplay = marketCapRaw ? formatNumber(marketCapRaw, '$', ' B') : 'N/A'; return { marketCap: marketCapDisplay, items: [{ label: 'Revenue', value: formatNumber(metrics.revenuePerShareTTM, '$') }, { label: 'Market Cap', value: marketCapDisplay }, { label: 'P/E Ratio', value: formatNumber(metrics.peTTM) }, { label: 'EPS', value: formatNumber(metrics.epsTTM, '$') }, { label: 'Dividend', value: formatNumber(metrics.dividendYieldIndicatedAnnual, '', '%') }, { label: 'Beta', value: formatNumber(metrics.beta) }] }; }
